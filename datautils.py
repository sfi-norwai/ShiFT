...
def _get_time_features(dt):
    return np.stack([
            ...
            dt.month.to_numpy(),
            dt.isocalendar().week.to_numpy(),  # Updated from weekofyear
        ], axis=1).astype(float)  # Use float instead of np.float


def load_forecast_csv(name, univar=False):
    data = pd.read_csv(f'datasets/forecast_datasets/{name}.csv', index_col='date', parse_dates=True)
    ...
    if name in ('ETTh1', 'ETTh2', 'electricity', 'weather', 'exchange_rate'):
