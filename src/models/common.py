...
class TransformerContext(nn.Module):
    def __init__(self, dim, num_layers=2, nhead=4):
        super().__init__()
        layer = TransformerEncoderLayer(d_model=dim, nhead=nhead, batch_first=True)
        self.encoder = TransformerEncoder(layer, num_layers=num_layers)

    def forward(self, zt):
        # zt: (B, T, D)
        h = self.encoder(zt)
        ct = h[:, -1, :]  # last timestep as context summary
        return ct

class TCNContext(nn.Module):
    ...
    def forward(self, zt):
        # zt: (B, T, D)
        x = zt.transpose(1, 2)             # -> (B, D, T)
        h = self.network(x)                # -> (B, H, T)
        ct = h[:, :, -1]                   # take final timestep context -> (B, H)
        return ct
...
def manual_pad(x, min_len):
    pad_len = min_len - x.size(-1)
    pad_left = pad_len // 2
    pad_right = pad_len - pad_left
    # safest: just pad with zeros
