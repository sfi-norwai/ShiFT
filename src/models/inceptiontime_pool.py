...
class InceptionTime(nn.Module):
    ...
    def __init__(
        ...
        self.n_in_channels = 128
        self.instance_encoder = nn.Sequential(
            InceptionBlock(128, out_channels=out_channels, padding_mode=padding_mode),
            InceptionBlock(out_channels * 4, out_channels, padding_mode=padding_mode),
        )
        self.gap = nn.AdaptiveAvgPool1d(1)
        self.input_fc = nn.Linear(n_in_channels, 128)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = x.transpose(1,2)
        out = self.instance_encoder(x)
        out = self.gap(out)             # (B, C, 1)
        out = out.squeeze(-1)
