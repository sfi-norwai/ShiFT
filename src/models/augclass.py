...
class jitter():
    def __init__(self, device, sigma=0.3) -> None:
        self.sigma = sigma
        self.device = device
    def __call__(self,x):
...
class scaling():
    def __init__(self, device,sigma=0.5) -> None:
        self.sigma = sigma
        self.device = device
    def __call__(self,x):
...
class time_warp():
    def __init__(self, device,n_speed_change=100, max_speed_ratio=10) -> None:
        self.device = device
    ...
    def __call__(self, x_torch):
        ...
        return totensor(x_tran.astype(np.float32)).to(self.device)

class magnitude_warp():

    def __init__(self, device,n_speed_change:int =100, max_speed_ratio=10) -> None:
        self.device = device
        self.transform = tsaug.TimeWarp(n_speed_change=n_speed_change, max_speed_ratio=max_speed_ratio)
