...
class InfoNCE(nn.Module):
    def __init__(self, gar_hidden, genc_hidden, batch_size,device):
        ...
        self.prediction_step = 12 # Time steps k to predict into future
        self.negative_samples = 10 # Number of negative samples to be used for training
        self.subsample = True

        # predict |prediction_step| timesteps into the future
        self.predictor = nn.Linear(
            gar_hidden, genc_hidden * self.prediction_step, bias=False
        ).to(device)
    ...
    def infonce_loss(self, Wc, z, full_z):
        ...
        true_labels = torch.zeros((seq_len * self.batch_size,)).long()
        ...
        for k in range(1, self.prediction_step + 1):
            ...
            if False:
                ...
                correct = (
                    (predicted == true_labels[: (seq_len - k) * self.batch_size])
                    .sum()
                    .item()
                )
